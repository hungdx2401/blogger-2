	...
	b.fn.uploadFile = function(f) {			
		...
		(function l() {
			if (b.fn.ajaxForm) {				
				if (p.dragDrop) {
					var s = b(
							'<div class="' + p.dragDropContainerClass
									+ '" style="vertical-align:top;"></div>')
							.width(p.dragdropWidth);
					b(n).before(s);
					b(s).append(q);
		...
		function u() {
			if (p.afterUploadAll && !m) {
				...
				(function s() {
					if (n.sCounter != 0
							&& (n.sCounter + n.fCounter == n.tCounter)) {
						p.afterUploadAll(n);
						m = false
					} else {
		...
		function c(z, x, y) {
			...
			y.on("dragover", function(A) {
				...
				if (s.hasClass(x.dragDropContainerClass)
						&& !s.hasClass(x.dragDropHoverClass)) {
			...
			y.on("drop", function(A) {
				...
				if (!x.multiple && s.length > 1) {
					if (x.showError) {
						b(
								"<div class='" + x.errorClass + "'>"
										+ x.multiDragErrorStr + "</div>")
								.appendTo(z.errorLog)
					}
					return
		...
		function r(K, E, x) {
			for (var F = 0; F < x.length; F++) {
				if (!g(E, K, x[F].name)) {
					if (K.showError) {
						b(
								"<div class='" + K.errorClass + "'><b>"
										+ x[F].name + "</b> " + K.extErrorStr
										+ K.allowedTypes + "</div>").appendTo(
								E.errorLog)
					}
					continue
				}
				...
				if (K.maxFileCount != -1 && E.selectedFiles >= K.maxFileCount) {
					if (K.showError) {
						b(
								"<div class='" + K.errorClass + "'><b>"
										+ x[F].name + "</b> "
										+ K.maxFileCountErrorStr
										+ K.maxFileCount + "</div>").appendTo(
								E.errorLog)
					}
					continue
				}
				...
				H.filename.html(J);
				var y = b("<form style='display:block; position:absolute;left: 150px;' class='"
						+ E.formGroup
						+ "' method='"
						+ K.method
						+ "' action='"
						+ K.url + "' enctype='" + K.enctype + "'></form>");
				y.appendTo("body");
				var A = [];
				A.push(x[F].name);
				w(y, G, H, A, E, x[F]);
		...
		function o(A, y) {
			...
			if (A.existingFileNames.length) {
				for (var s = 0; s < A.existingFileNames.length; s++) {
					if (A.existingFileNames[s] == y
							|| p.duplicateStrict
							&& A.existingFileNames[s].toLowerCase() == y
									.toLowerCase()) {
						z = true
					}
				}
		...
		function k(D, C, z, x) {
			$.get("/_blobstore/get-upload-url", function(resp) {			
				z.url = resp;
			});
			var E = "ajax-upload-id-" + (new Date().getTime());
			var B = b("<form method='" + z.method + "' action='" + z.url
					+ "' enctype='" + z.enctype + "'></form>");
			var y = "<input type='file' id='" + E + "' name='" + z.fileName
					+ "' accept='" + z.acceptFiles + "'/>";
			if (z.multiple) {
				if (z.fileName.indexOf("[]") != z.fileName.length - 2) {
					z.fileName += "[]"
				}
				y = "<input type='file' id='" + E + "' name='" + z.fileName
						+ "' accept='" + z.acceptFiles + "' multiple/>"
			}
				...
				if (a.fileapi && a.formdata) {
					...
					for (var I = 0; I < H.length; I++) {
						if (z.showFileCounter) {
							F += D.fileCounter + z.fileCounterStyle + H[I]
									+ "<br>"
						} else {
							F += H[I] + "<br>"
						}
					...
					if (z.maxFileCount != -1
							&& (D.selectedFiles + H.length) > z.maxFileCount) {
						if (z.showError) {
							b(
									"<div class='" + z.errorClass + "'><b>" + F
											+ "</b> " + z.maxFileCountErrorStr
											+ z.maxFileCount + "</div>")
									.appendTo(D.errorLog)
						}
						return
		...
		function j(y, x) {
			this.statusbar = b("<div class='ajax-file-upload-statusbar'></div>")
					.width(x.statusBarWidth);
			this.preview = b("<img class='ajax-file-upload-preview' />").width(
					x.previewWidth).height(x.previewHeight).appendTo(
					this.statusbar).hide();
			...
			this.progressbar = b(
					"<div class='ajax-file-upload-bar " + y.formGroup
							+ "'></div>").appendTo(this.progressDiv);
			this.abort = b(
					"<div class='ajax-file-upload-red " + x.abortButtonClass
							+ " " + y.formGroup + "'>" + x.abortStr + "</div>")
					.appendTo(this.statusbar).hide();
			this.cancel = b(
					"<div class='ajax-file-upload-red " + x.cancelButtonClass
							+ " " + y.formGroup + "'>" + x.cancelStr + "</div>")
					.appendTo(this.statusbar).hide();
			this.done = b(
					"<div class='ajax-file-upload-green'>" + x.doneStr
							+ "</div>").appendTo(this.statusbar).hide();
			this.download = b(
					"<div class='ajax-file-upload-green'>" + x.downloadStr
							+ "</div>").appendTo(this.statusbar).hide();
			this.del = b(
					"<div class='ajax-file-upload-red'>" + x.deletelStr
							+ "</div>").appendTo(this.statusbar).hide();
			if (x.showQueueDiv) {
		...
		function w(D, C, x, z, E, B) {			
			...
			var y = {
				...
				beforeSubmit : function(J, G, I) {
					...
					x.statusbar.append("<div class='" + C.errorClass + "'>"
							+ C.uploadErrorStr + "</div>");
					x.cancel.show();
					D.remove();
				...
				success : function(F, s, H) {
					if (C.returnType == "json" && b.type(F) == "object"
							&& F.hasOwnProperty(C.customErrorKeyStr)) {
						x.abort.hide();
						var G = F[C.customErrorKeyStr];
						C.onError.call(this, z, 200, G, x);
						if (C.showStatusAfterError) {
							x.progressDiv.hide();
							x.statusbar.append("<span class='" + C.errorClass
									+ "'>ERROR: " + G + "</span>")
						} else {
							x.statusbar.hide();
							x.statusbar.remove()
